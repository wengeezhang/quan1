...
支持三阶段生成：
  Phase 1: 角色基准肖像（正面半身像，纯文生图）
  Phase 2: 角色其余图片（以基准肖像为参考图，图生图，保证外观一致性）
  Phase 3: 场景 + 道具（纯文生图）
...
# 肖像视角名称（按星级）
PORTRAIT_VIEWS = {
    6: ["正面半身像", "Character Sheet"],
    5: ["正面半身像", "Character Sheet"],
    4: ["正面半身像", "Character Sheet"],
    3: ["正面半身像"],
}
...
def main():
    ...
    parser.add_argument("--phase", type=int, default=0, choices=[0, 1, 2, 3],
                        help="生成阶段: 1=角色基准肖像, 2=角色参考图生图, 3=场景+道具, 0=全部 (默认: 0)")
    ...
    if args.phase == 1:
        ...
        log.info(f"Phase 1 模式: {len(tasks)} 个任务（角色基准肖像）")
    elif args.phase == 2:
        tasks = [t for t in tasks if t["phase"] == 2]
        log.info(f"Phase 2 模式: {len(tasks)} 个任务（角色参考图生图）")
    elif args.phase == 3:
        tasks = [t for t in tasks if t["phase"] == 3]
        log.info(f"Phase 3 模式: {len(tasks)} 个任务（场景 + 道具）")
    ...
    if args.dry_run:
        ...
        # 统计摘要
        by_type = {}
        by_phase = {1: 0, 2: 0, 3: 0}
        for t in tasks:
            by_type[t["type"]] = by_type.get(t["type"], 0) + 1
            by_phase[t["phase"]] = by_phase.get(t["phase"], 0) + 1
        print(f"\n摘要: ", end="")
        print(", ".join(f"{k}: {v}" for k, v in by_type.items()))
        print(f"Phase 1 (角色基准肖像): {by_phase[1]}, Phase 2 (角色参考图生图): {by_phase[2]}, Phase 3 (场景+道具): {by_phase[3]}")
    ...
    for i, t in enumerate(tasks, 1):
        portrait_tag = " [肖像]" if t["is_portrait"] else ""
        phase_tag = {1: " [基准肖像]", 2: " [参考图生图]", 3: " [场景/道具]"}.get(t["phase"], "")
