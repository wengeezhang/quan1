...
{从小说原文精确提取：台词内容、说话人、出现时间、语气。无对白写"无"。时间点应对齐"动态描述"的节拍，供 Seedance 视频生成时口型/音轨同步使用。}

| 时间点 | 说话人 | 台词（原文） | 语气/交付 |
|--------|--------|------------|----------|
| 0-3s | {角色A} | "{原文台词}" | {如"疲惫低声"、"急促怒喝"、"犹豫试探"} |
| 4-7s | {角色B} | "{原文台词}" | {...} |
...
5. **对白从小说原文精确提取**：读对应章节原文，识别本场次内的对话内容，将每句台词分配到合适的镜头，标注说话人、时间点、语气；无对白镜头标注"无"
6. 标注首帧合成提示（连续性需求、参考图使用建议）
7. 产出 `storyboard/{SC{NNN}}.md`
...
3. **对白** → Seedance 口型同步与音轨生成（台词原文+时间点+说话人+语气）
4. **时长** → Seedance 生成参数
5. **音效提示** → Seedance 音频生成引导（如支持）
...
| 对白段遗漏或伪造 | 视频生成无法同步口型/音轨，或生成不符原著的台词 | 每个有对白的镜头必须从小说原文精确提取台词，标注说话人+时间点+语气 |
...
9. **对白准确**：每个含台词的镜头都从小说原文精确提取台词，附带说话人、时间点、语气标注；无对白镜头标注"无"
10. **下游可用**：阶段6可直接从分镜脚本中提取画面描述+stage_id→参考图路径；阶段7可直接提取运镜方式、动态描述、对白台词；阶段8可直接按镜头顺序+衔接方式进行剪辑
