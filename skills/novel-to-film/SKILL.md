...
- 每个元素圣经必须包含**阶段注册表（Stage Registry）**，为该元素在故事中的每个视觉阶段定义唯一的 `stage_id`。这个 `stage_id` 是后续阶段4（资产目录名）、阶段5（分镜标注）、阶段6（参考图查找）的精准映射键，全流程一个字都不能改
...
角色在不同故事阶段有不同的外貌（如朱围庸：灵魂期 → 占身丁路期 → 占身乞丐期），场景在不同时段有不同氛围。资产库以阶段3各元素圣经中**阶段注册表（Stage Registry）**定义的 `stage_id` 为组织单元，而非章节号。`stage_id` 是贯穿阶段3→4→5→6的**单一真相源**——圣经中注册的 `stage_id` = 资产目录文件夹名 = asset_index.md 索引键 = 分镜脚本角色/场景标注 = 首帧合成参考图查找键。**一个字都不能差。**
